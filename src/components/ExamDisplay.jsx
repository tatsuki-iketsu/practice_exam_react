...
let c_ansCheck = [];        // 正誤を確認有無を格納

const ExamDisplay = (props) =>{
  const [examData,setExamData] = useState(Data);                     // 問題を格納
  const [examButtonDisabled,setExamButtonDisabled] = useState([]);               // 問題の回答ボタン有効無効を格納
  ...
  function closeModal() {
    // Modalを閉じる前にすべてのボタンを有効化（disabled=false)にする
    setExamButtonDisabled(examButtonDisabled.map((disabled) => (disabled = false)));      
    setIsOpen(false);
  }
  ...
  // 内側シャッフル　TODO：Express→MySQLのストアドに移行予定
  ...
  const shuffleExam1=(examData,InitialDisplay)=> {
    ...
    for (let i = examData.setExams.length - 1; i > 0; i--) {
      ...
      // TODO 全問回答後確認機能時は、Modalではなく画面に下記群をセットする
      // 回答、正解不正解、解説の格納
      let c_examDisabled = [];
      let c_tipDisplay = [];
      let c_ansDisplay = [];
      ...
      setExamButtonDisabled(c_examDisabled);
  ...
  ////// 複数回答の表示関数 //////
  // TODO 単一回答の問題はチェックボックスではなくラジオボタンに変更予定
  const selectedAnserGet = (selectedChecks,setExam,index,examId,tipURI) => {
    ...
    // ボタン非活性化とModalに表示させる解説をセット
    // Modalを閉じるまでボタンを無効化
    setExamButtonDisabled(examButtonDisabled.map((disabled, i) => (i === index ? true : disabled)));
    // 正誤確認済みをセット 
        ...
        {setExam.selectAnswers.map((Answer) => (
          <>
            <div className="marginTopBottom"><nobr>
              ...
              {selectTipsDisplay(Answer.selecttips,examButtonDisabled[index])}
              </div>
          </>
          ))}      
        </form>
        <button disabled={examButtonDisabled[index]} onClick={()=>selectedCheck(setExam,setExam.setItem,index,setExam.examId,setExam.TipURI)}  class="selectedCheck">回答を確認</button>
      </div>
